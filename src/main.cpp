...
void write_intro_audio_probe(const std::filesystem::path& root, std::ostream& output) {
  ...
  const auto inventory = off::graphics::IntroAudioCueInventory::from_prepared(
      resources.sounds(), resources.audio()->header(), resources.audio()->record_indices());
  output << "intro-audio-probe=completed\n"
         << "intro-audio-source-owners=" << inventory.source_owner_count << '\n'
         << "intro-audio-distinct-sound-definitions="
         << inventory.distinct_sound_definition_count << '\n'
...
int run_first_cut_probe(const std::filesystem::path &data_path, bool readiness_only,
  ...
  // Do not let successful construction itself be reported as readiness. The
  // handoff is a relation-only value and must leave the player demonstrably
  // cold before this probe emits its fixed verification marker.
  if(first_cut->initialization().phase_one_complete() ||
      first_cut->initialization().phase_two_complete() || first_cut->receiver().open() ||
      first_cut->receiver().closed())
    throw std::runtime_error("first-cut cold probe handoff changed player lifecycle state");
...
int main(int argc, char **argv) {
  ...
  if (probe_intro_audio) {
    ...
    try {
      write_intro_audio_probe(verification.root, std::cout);
      return 0;
    } catch (const std::exception& error) {
