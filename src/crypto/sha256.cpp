...
[[nodiscard]] constexpr std::uint32_t load_be32(const std::byte* bytes) {
...
void store_be64(std::byte* output, std::uint64_t value) {
...
void Sha256::update(std::span<const std::byte> bytes) {
    ...
    while (!bytes.empty()) {
        ...
        std::copy_n(bytes.begin(), amount, buffer_.begin() + static_cast<std::ptrdiff_t>(buffered_));
        buffered_ += amount;
...
Sha256Digest Sha256::finish() {
    ...
    store_be64(buffer_.data() + 56, bit_length);
    transform(buffer_.data());
    finished_ = true;
...
Sha256Digest sha256_file(const std::filesystem::path& path) {
    ...
    if (!input.eof()) {
        throw std::runtime_error("error while hashing file");
    }
    return hasher.finish();
}
