...
constexpr std::uint16_t texture_id_mask = 0x07ffU;
constexpr std::uint16_t texture_selector_flag = 0x0800U;
constexpr std::uint16_t texture_selector_mask =
    texture_id_mask | texture_selector_flag;

void require_data_offset(std::uint32_t value, std::size_t descriptor_offset,
                         bool optional) {
  if (optional && value == 0) {
    return;
  }
  if (value < header_size || value >= descriptor_offset || (value & 1U) != 0U) {
    throw std::runtime_error(
        "primitive descriptor contains an invalid data offset");
  }
}
...
[[nodiscard]] float read_float(const ByteReader &reader, std::size_t offset) {
  ...
  if (!std::isfinite(value)) {
    throw std::runtime_error(
        "primitive vertex contains a non-finite component");
  }
  return value;
}
...
PrimitiveCatalog PrimitiveCatalog::parse(std::span<const std::byte> bytes) {
  ...
  for (std::uint32_t entry_index = 0; entry_index < entry_count;
    ...
    const auto packed_index =
        reader.u32(static_cast<std::size_t>(index_offset) + entry_index * 4U);
    ...
    if (descriptor_offset < header_size || (descriptor_offset & 1U) != 0U ||
      ...
      throw std::runtime_error(
          "primitive index contains an invalid descriptor offset");
    }
    ...
    entry.texture_selector = reader.u16(descriptor_offset + 4U);
    if ((entry.texture_selector & ~texture_selector_mask) != 0U ||
        reader.u16(descriptor_offset + 6U) != 0U) {
      throw std::runtime_error(
          "primitive descriptor contains an invalid texture selector");
    }
    const auto texture_id = entry.texture_selector & texture_id_mask;
    if (texture_id != 0U) {
      entry.texture_id = texture_id;
    }
    entry.texture_selector_flagged =
        (entry.texture_selector & texture_selector_flag) != 0U;
    ...
    if ((entry.primitive_kind != 0 && entry.primitive_kind != 3) ||
        ...
        topology_word_count == 0 ||
        topology_word_count > maximum_topology_words) {
      throw std::runtime_error(
          "primitive descriptor contains unsupported counts or kind");
    }
    ...
    if (static_cast<std::uint64_t>(entry.vertex_data_offset) + vertex_bytes >
        descriptor_offset) {
      throw std::runtime_error(
          "primitive vertex table exceeds descriptor bounds");
    }
    const auto topology_bytes =
        static_cast<std::uint64_t>(topology_word_count) * sizeof(std::uint16_t);
    if (static_cast<std::uint64_t>(entry.topology_data_offset) +
            topology_bytes >
        descriptor_offset) {
    ...
    for (std::uint32_t batch_index = 0; batch_index < batch_count;
      ...
      if (word_cursor >= topology_word_count) {
        throw std::runtime_error(
            "primitive topology is missing an index count");
      }
      const auto index_count =
          reader.u16(entry.topology_data_offset +
                     static_cast<std::size_t>(word_cursor) * 2U);
      ++word_cursor;
      if (index_count == 0 || index_count > topology_word_count - word_cursor) {
        throw std::runtime_error(
            "primitive topology batch exceeds its word table");
      }
      ...
      for (std::uint32_t index = 0; index < index_count; ++index) {
        ...
        if (vertex_index >= entry.vertex_count) {
          throw std::runtime_error(
              "primitive topology references a missing vertex");
        }
  ...
  if (std::adjacent_find(descriptor_offsets.begin(), descriptor_offsets.end(),
                         [](std::uint32_t first, std::uint32_t second) {
                           return second - first < descriptor_size;
                         }) != descriptor_offsets.end()) {
    throw std::runtime_error(
        "primitive index contains overlapping descriptors");
  }
  return result;
}
