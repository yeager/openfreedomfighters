...
namespace {
// Retail display text is deliberately confined to this implementation.  The
// public cache result reduces this private snapshot to text-free metadata.
  ...
  for (std::size_t cursor{}; cursor < value.size();) {
    ...
    const unsigned length = lead >= 0xc2U && lead <= 0xdfU   ? 2U
                            ...
                            : lead >= 0xf0U && lead <= 0xf4U ? 4U
                                                             : 0U;
    if (length == 0U || length > value.size() - cursor)
      return false;
    std::uint32_t code_point = lead & (length == 2U   ? 0x1fU
                                       : length == 3U ? 0x0fU
                                                      : 0x07U);
...
bool valid_source_text(std::string_view value) noexcept {
  return value.size() <= maximum_text_bytes &&
...
bool valid_snapshot(const RetailLocalizationSnapshot &snapshot) noexcept {
  if (snapshot.installation_identity.size() > 128U ||
      snapshot.parser_identity.size() > 128U ||
      snapshot.source_set.size() > 96U ||
      !valid_identifier(snapshot.installation_identity) ||
      !valid_identifier(snapshot.parser_identity) ||
      !valid_identifier(snapshot.source_set) || snapshot.strings.empty() ||
      snapshot.strings.size() > maximum_strings)
    return false;
  std::size_t total =
      magic.size() + 8U + snapshot.installation_identity.size() + 8U +
  ...
  for (std::size_t index{}; index < snapshot.strings.size(); ++index) {
    if (snapshot.strings[index].ordinal != index ||
        !valid_source_text(snapshot.strings[index].english) ||
        total > maximum_catalog_bytes - 16U ||
        snapshot.strings[index].english.size() >
            maximum_catalog_bytes - total - 16U)
      return false;
    total += 16U + snapshot.strings[index].english.size();
  }
  return true;
}
std::optional<RetailLocalizationMetadata>
metadata_from_snapshot(const RetailLocalizationSnapshot &snapshot) noexcept {
  if (!valid_snapshot(snapshot) ||
      snapshot.strings.size() > std::numeric_limits<std::uint64_t>::max())
    return std::nullopt;
  return RetailLocalizationMetadata{
      snapshot.installation_identity, snapshot.parser_identity,
      snapshot.source_set, 0U,
      static_cast<std::uint64_t>(snapshot.strings.size())};
}
void append_u64(std::string &out, std::uint64_t value) {
  for (unsigned byte{}; byte < sizeof(value); ++byte)
    out.push_back(static_cast<char>((value >> (byte * 8U)) & 0xffU));
}
std::optional<std::uint64_t> read_u64(std::string_view input,
                                      std::size_t &cursor) {
  if (cursor > input.size() || sizeof(std::uint64_t) > input.size() - cursor)
    return std::nullopt;
  std::uint64_t value{};
  for (unsigned byte{}; byte < sizeof(value); ++byte)
    value |=
...
std::string encode(const RetailLocalizationSnapshot &snapshot) {
  ...
  append_blob(out, snapshot.installation_identity);
  append_blob(out, snapshot.parser_identity);
  append_blob(out, snapshot.source_set);
...
std::optional<RetailLocalizationSnapshot> decode(std::string_view input) {
  ...
  auto source_set = read_blob(input, cursor, 96U);
  const auto count = read_u64(input, cursor);
  if (!installation || !parser || !source_set || !count || *count == 0U ||
      *count > maximum_strings)
    return std::nullopt;
  RetailLocalizationSnapshot result{
      std::move(*installation), std::move(*parser), std::move(*source_set), {}};
  result.strings.reserve(static_cast<std::size_t>(*count));
  for (std::uint64_t index{}; index < *count; ++index) {
    const auto ordinal = read_u64(input, cursor);
    auto english = read_blob(input, cursor, maximum_text_bytes);
    if (!ordinal || !english)
      return std::nullopt;
    result.strings.push_back({*ordinal, std::move(*english)});
  }
  if (cursor != input.size() || !valid_snapshot(result))
    return std::nullopt;
  return result;
}
...
std::optional<RetailLocalizationSnapshot>
load(const std::filesystem::path &path) {
  if (!regular_non_link(path))
    return std::nullopt;
  std::error_code error;
  const auto size = std::filesystem::file_size(path, error);
  if (error || size > maximum_catalog_bytes)
    return std::nullopt;
...
bool store(const std::filesystem::path &path,
           const RetailLocalizationSnapshot &snapshot) {
  std::error_code error;
  ...
  if (error || !directory_non_link(parent))
    return false;
  error.clear();
  ...
  const auto nonce = crypto::to_hex(crypto::sha256(
      std::to_string(
          std::chrono::steady_clock::now().time_since_epoch().count()) +
      ":" + std::to_string(random()) + ":" +
      std::to_string(staging_counter++)));
  const auto staging_directory = parent / (".staging-" + nonce);
  if (!std::filesystem::create_directory(staging_directory, error) || error)
    return false;
  std::filesystem::permissions(staging_directory,
                               std::filesystem::perms::owner_all,
                               std::filesystem::perm_options::replace, error);
  if (error) {
    std::filesystem::remove(staging_directory, error);
    return false;
  }
...
std::string make_retail_string_id(std::string_view source_set,
                                  std::uint64_t ordinal) {
  if (!valid_identifier(source_set) || source_set.size() > 96U)
    return {};
  return "off.retail." + std::string{source_set} + "." +
         std::to_string(ordinal);
}
std::optional<RetailTranslationCatalog>
RetailTranslationCatalog::build(std::vector<RetailTranslationEntry> entries) {
  if (entries.empty())
    return std::nullopt;
  for (const auto &entry : entries)
    if (!valid_identifier(entry.id) || !valid_translation_text(entry.text))
      return std::nullopt;
  std::ranges::sort(entries, {}, &RetailTranslationEntry::id);
  if (std::ranges::adjacent_find(entries, {}, &RetailTranslationEntry::id) !=
      entries.end())
    return std::nullopt;
...
RetailTranslationCatalog::find(std::string_view id) const noexcept {
  const auto found =
      std::ranges::lower_bound(entries_, id, {}, &RetailTranslationEntry::id);
  if (found == entries_.end() || found->id != id)
    return std::nullopt;
  return found->text;
}
RetailLocalizationCacheResult ensure_retail_localization_metadata(
    const std::filesystem::path &root, std::string_view installation_identity,
    std::string_view parser_identity, std::string_view source_set,
    const std::function<std::optional<std::vector<RetailSourceString>>()>
        &extract) {
  if (root.empty() || !valid_identifier(installation_identity) ||
      !valid_identifier(parser_identity) || !valid_identifier(source_set) ||
      source_set.size() > 96U || !extract)
    return {.status = RetailLocalizationCacheStatus::unavailable,
  ...
  if (root_error || !directory_non_link(root) ||
    ...
    return {.status = RetailLocalizationCacheStatus::unavailable,
            .metadata = std::nullopt};
  const auto path =
      cache_path(root, installation_identity, parser_identity, source_set);
  if (const auto existing = load(path);
      existing && existing->installation_identity == installation_identity &&
      existing->parser_identity == parser_identity &&
      existing->source_set == source_set) {
    const auto metadata = metadata_from_snapshot(*existing);
    if (metadata)
      return {.status = RetailLocalizationCacheStatus::loaded,
              .metadata = *metadata};
  }
  std::error_code error;
  if (std::filesystem::exists(path, error)) {
    if (error || !regular_non_link(path) ||
  ...
  if (!extracted)
    return {.status = RetailLocalizationCacheStatus::unavailable,
            .metadata = std::nullopt};
  RetailLocalizationSnapshot snapshot{std::string{installation_identity},
                                      std::string{parser_identity},
                                      std::string{source_set}, *extracted};
  const auto metadata = metadata_from_snapshot(snapshot);
  if (!metadata)
    return {.status = RetailLocalizationCacheStatus::invalid,
            .metadata = std::nullopt};
  if (!store(path, snapshot))
    return {.status = RetailLocalizationCacheStatus::unavailable,
            .metadata = std::nullopt};
