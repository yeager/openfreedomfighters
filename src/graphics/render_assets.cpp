...
RenderAssetBindings
RenderAssetBindings::build(std::span<const data::PrimitiveEntry> primitives,
  ...
  for (std::size_t index = 0; index < textures.size(); ++index) {
    ...
    if (id >= texture_indexes.size() || texture_indexes[id] != missing) {
      throw std::runtime_error(
          "render assets contain a duplicate or invalid texture ID");
    }
  ...
  for (std::size_t index = 0; index < primitives.size(); ++index) {
    ...
    for (const auto &batch : primitive.batches) {
      ...
      binding.indices.insert(binding.indices.end(), batch.indices.begin(),
                             batch.indices.end());
    }
    ...
    if (primitive.texture_id.has_value()) {
      if (*primitive.texture_id >= texture_indexes.size()) {
        throw std::runtime_error(
            "render primitive references an invalid texture ID");
      }
      const auto texture_index = texture_indexes[*primitive.texture_id];
      if (texture_index == missing) {
        throw std::runtime_error(
            "render primitive references a missing texture image");
      }
