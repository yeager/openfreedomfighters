...
public:
  BoundLifecycleServices(std::shared_ptr<NormalIntroSceneSession> session,
    ...
    if (!session_ || !config_.enter_global_lifecycle)
      throw std::runtime_error("normal intro lifecycle adapter requires an "
                               "owned session and global service");
  ...
  void reader_bracket() {
    try {
      ...
      if (session_->stage() != NormalIntroSceneSessionStage::postconstructed)
        throw std::runtime_error(
            "normal intro lifecycle adapter has no postconstruction receipt");
      session_->complete_postconstruction_reader_bracket(
          config_.retained_saved_value);
      if (session_->stage() !=
              NormalIntroSceneSessionStage::reader_bracket_complete ||
          session_->runtime().reader_bracket_stage() !=
              IntroReaderBracketStage::ordinary_reader_boundary_complete)
        throw std::runtime_error(
            "normal intro lifecycle adapter reader receipt was not completed");
      stage_ = Stage::reader_complete;
    } catch (...) {
      stage_ = Stage::failed;
      throw;
  ...
  void outer_loader_tail() {
    try {
      ...
      if (session_->stage() !=
          NormalIntroSceneSessionStage::reader_bracket_complete)
        throw std::runtime_error(
            "normal intro lifecycle adapter reader receipt was lost");
      ...
      if (session_->stage() !=
              NormalIntroSceneSessionStage::outer_loader_tail_complete ||
          session_->runtime().outer_loader_tail_stage() !=
              IntroOuterLoaderTailStage::second_saved_pass_complete)
        throw std::runtime_error("normal intro lifecycle adapter outer-tail "
                                 "receipt was not completed");
  ...
  void enter_global_lifecycle() {
    try {
      ...
      if (session_->stage() !=
          NormalIntroSceneSessionStage::outer_loader_tail_complete)
        throw std::runtime_error(
            "normal intro lifecycle adapter outer-tail receipt was lost");
      if (!session_->runtime().preflight_global_lifecycle().ready())
        throw std::runtime_error(
            "normal intro lifecycle adapter global lifecycle is not admitted");
      config_.enter_global_lifecycle(*session_);
      if (session_->runtime().components().failed() ||
          !session_->runtime().components().phases_completed())
        throw std::runtime_error(
            "normal intro lifecycle adapter global lifecycle did not complete");
...
NormalIntroSceneLifecycleServiceAdapters::bind(
  ...
  auto bound = std::make_shared<BoundLifecycleServices>(std::move(session),
                                                        std::move(config));
  return {
      .reader_bracket = [bound] { bound->reader_bracket(); },
...
NormalIntroSceneLifecycleServiceAdapters::bind_after_reader_bracket(
    std::shared_ptr<NormalIntroSceneSession> session,
    NormalIntroSceneLifecycleAdapterConfig config) {
  auto bound = std::make_shared<BoundLifecycleServices>(
      std::move(session), std::move(config), true);
  return {[bound] { bound->outer_loader_tail(); },
          [bound] { bound->enter_global_lifecycle(); }};
}
