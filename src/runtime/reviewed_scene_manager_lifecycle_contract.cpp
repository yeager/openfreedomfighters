...
namespace {
constexpr std::string_view filename{"reviewed-scene-manager-lifecycle.json"};
 ...
 private:
  ...
  [[nodiscard]] std::optional<std::string> string() {
    ...
    while (cursor_ < source_.size()) {
      ...
      if (character == '\\' || byte < 0x20U || byte > 0x7EU || result.size() >= 96U) {
        return std::nullopt;
      }
...
[[nodiscard]] bool record(const Json::Object& value, bool success) {
  constexpr std::array keys{
      "manager_entered", "scene_staged", "global_lifecycle_completed",
      "component_phase_one_completed", "camera_route_ready", "scene_committed", "outcome"};
  return exact_keys(value, keys) && equals(value, "manager_entered", true) &&
      equals(value, "scene_staged", success) &&
      equals(value, "global_lifecycle_completed", success) &&
      equals(value, "component_phase_one_completed", success) &&
      equals(value, "camera_route_ready", success) &&
      equals(value, "scene_committed", success) &&
...
[[nodiscard]] bool validated(const Json& root) {
  ...
  if (!top || !exact_keys(*top, std::array{"format", "candidate", "repeat", "failure"}) ||
      !equals(*top, "format", "off.scene-manager-lifecycle-contract/v1")) {
    return false;
  }
...
std::optional<ReviewedSceneManagerLifecycleContract>
ReviewedSceneManagerLifecycleContract::load_local(const std::filesystem::path& local_directory) {
  ...
  return ReviewedSceneManagerLifecycleContract{
