#!/usr/bin/env python3
"""Collect two private, fresh-isolated ParamAnim reader observations.

The executable supplied here is an operator-maintained private observer, not
part of OpenFreedomFighters. This wrapper supplies no game location, process
identifier, target selector, debugger argument, or shell. It creates an empty
private workspace, accepts exactly two bounded structural records, then retains
only their sanitized forms and their repeat-gated source-free result.
"""
...
import paramanim_deferred_reader_repeat_pair as repeat_pair


REPOSITORY_ROOT = pathlib.Path(__file__).resolve().parent.parent
FIRST_RAW_NAME = "first.raw.json"
SECOND_RAW_NAME = "second.raw.json"
FIRST_SANITIZED_NAME = "first.sanitized.json"
SECOND_SANITIZED_NAME = "second.sanitized.json"
REPEAT_PAIR_NAME = "repeat-pair.json"
_RAW_NAMES = (FIRST_RAW_NAME, SECOND_RAW_NAME)
_EXPECTED_RAW_FILES = frozenset(_RAW_NAMES)
MAX_RAW_RECORD_BYTES = observation.MAX_OBSERVATION_BYTES
DEFAULT_TIMEOUT_SECONDS = 300
MAX_TIMEOUT_SECONDS = 1800


def _outside_repository(path: pathlib.Path, label: str) -> pathlib.Path:
    """Reject symlinks before canonicalizing an operator-supplied location."""
...
def _discard_raw_records(workspace: pathlib.Path) -> None:
    """Remove only protocol entries; unknown private operator notes are untouched."""
    try:
...
def _collect(workspace: pathlib.Path) -> dict[str, Any]:
    """Sanitize both records, repeat-gate them, and erase raw input first."""
    descriptor = _open_workspace_no_follow(workspace)
    try:
        try:
            ...
            first = observation.sanitize_observation(_read_workspace_json(descriptor, FIRST_RAW_NAME))
            second = observation.sanitize_observation(_read_workspace_json(descriptor, SECOND_RAW_NAME))
...
def execute_observation(*, observer: pathlib.Path, workspace: pathlib.Path,
    ...
    """Launch only the private observer's literal fresh-isolated protocol."""
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"collected two fresh source-free ParamAnim observations ({len(pair['events'])} events)")
    return 0
