...
- `ANM` begins with `MNA\0`; word 1 is the file size with bit 31 set and word 2 is the file size. Word 3 is nine plus the number of reference tables (one through six across all 42 files), and word 4 is 10. Each reference table now has a native directory parser; animation payload records remain opaque.
...
The rules above were then promoted to validators and checked against the full corpus. `ZGF`, `SUP`, `GMS`, `SND`, `PRM`, and `ANM` pass every stated rule in every observed file. `TEX` passes the size and `3,4` rules in all 90 files; word 1 has an 88-file normal variant (`word 0 + 8,192`) and a two-file empty variant (`word 0`). `SUP`, `TEX`, and the `ANM` reference-table directory have native parsers with synthetic mutation tests; see [SCENE_SUPPORT_FORMAT.md](SCENE_SUPPORT_FORMAT.md) and [TEXTURE_FORMAT.md](TEXTURE_FORMAT.md). Animation records beyond the `ANM` directory remain opaque. The next step for the remaining families is to locate section offsets from these length equations and test mutations against synthetic fixtures.
