...
- `ANM` begins with `MNA\0`; word 1 is the file size with bit 31 set and word 2 is the file size. Word 3 is nine plus the number of reference tables (one through six across all 42 files), and word 4 is 10. Each reference table has a native directory parser. Its aligned values are retained as neutral reference words, not treated as physical offsets. A following length-delimited descriptor block contains 3 through 24 twelve-byte descriptors (457 total); its final descriptor has tag 7 and the first two words remain opaque. Eight tagged, length-delimited sections follow in the fixed order 7, 8, 6, 11, 13, 15, 4, 5 and consume the rest of every observed file. The first three expose validated per-reference component spans; section semantics remain opaque.
