...
| `BUF` | NUL-terminated object names and bounded auxiliary blocks referenced by GMS sources | per-type auxiliary schemas and downstream primitive links |
| `GMS` | object-source transforms, attachments, identifiers, source-class diagnostics, and runtime-pool population | remaining sections and per-type materialization semantics |
...
Scene render assets retain the complete parsed `ANM` metadata for their lifetime. This preserves table identities, neutral reference words, and opaque descriptors for a future evidence-backed object-pose binding instead of discarding them after archive validation.
