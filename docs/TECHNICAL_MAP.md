...
| `PRM` | indexed render primitives with grouped topology | vertex formats, materials, skinning |
...
Across all 90 archives, each of `GMS`, `OCT`, `PRM`, `RMC`, `RMI`, `SGP`, `SND`, `SUP`, `TEX`, and `ZGF` occurs 90 times. `BUF` and `LOC` occur 88 times and `ANM` occurs 42 times. This regularity strongly supports a per-scene resource-family design. `SUP` is confirmed as a dependency list, `TEX` is decoded to RGBA8, and `PRM` is parsed into indexed descriptors and grouped topology; the remaining family semantics are still hypotheses.
...
3. Resolve `PRM` vertex formats and map the `RMC`/`RMI` relationship, then bind decoded `TEX` pixels to renderable geometry.
