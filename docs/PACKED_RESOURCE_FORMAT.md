...
Every decoded `ZGF` payload begins with the same four-byte inner signature. Its embedded-resource directory is parsed separately and documented in [ZGF_FORMAT.md](ZGF_FORMAT.md). The decoded `GMS` object-source and identifier directories are documented in [GMS_FORMAT.md](GMS_FORMAT.md); the remaining inner sections are research targets.
