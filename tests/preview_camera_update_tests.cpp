...
int main() {
  try {
    ...
    const auto queue=[&](auto& live) {
      ...
      ++queued;
      rejects([&] { update.run(owner,live,input,[](auto&) {}); });
    };
    update.toggle_latch=true; update.movement_scale=-5; update.secondary_scale=0;
    update.run(owner,first,input,queue);
    ...
    update.run(owner,second,input,queue);
    ...
    update.run(owner,first,input,queue); // Guard is released; equal transform needs no queue.
    ...
    rejects([&] { clocked.update_preview_camera(owner,live,input,clock_queue); });
    clocked.reset_clock(); clocked.clock().assign_crt_mode(true);
    clocked.clock().set_rate(0);
    input.pointer={0,0}; input.raw_crt_delta=0;
    clocked.update_preview_camera(owner,live,input,clock_queue);
    ...
    coupled.run(owner,rotated,motion,[](auto&) {});
    motion.pointer={2,1};
    coupled.run(owner,rotated,motion,[](auto&) {});
    ...
    rejects([&] { coupled.run(owner,rotated,motion,[](auto&) {}); });
    ...
    for(const bool negative_basis:{false,true}) {
      ...
      signed_update.run(owner,signed_pose,sample,[](auto&) {});
