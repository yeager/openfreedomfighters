...
std::optional<std::vector<off::ui::l10n::RetailSourceString>>
synthetic_loc_records() {
  ...
  for (std::size_t index{}; index < decoded->values.size(); ++index)
    result.push_back(
        {static_cast<std::uint64_t>(index), decoded->values[index]});
  return result;
}
} // namespace
...
int main() {
  ...
  try {
    const auto root =
        std::filesystem::current_path() / "off-retail-l10n-cache-tests";
    std::error_code error;
    std::filesystem::remove_all(root, error);
    unsigned extraction_calls{};
    const auto extract =
        [&]() -> std::optional<std::vector<RetailSourceString>> {
      ++extraction_calls;
      return std::vector<RetailSourceString>{
          {0U, "Project-authored source one"},
          {1U, "Project-authored source two"}};
    };
    const auto first = ensure_retail_localization_metadata(
        root, "install-test-v1", "loc-parser-v1", "ff.loc.startup.v1", extract);
    check(first.status == RetailLocalizationCacheStatus::extracted &&
              first.metadata && extraction_calls == 1U,
          "first supported extraction publishes text-free metadata");
    const auto second = ensure_retail_localization_metadata(
        root, "install-test-v1", "loc-parser-v1", "ff.loc.startup.v1", extract);
    check(second.status == RetailLocalizationCacheStatus::loaded &&
              second.metadata && extraction_calls == 1U,
          "matching private catalog does not re-extract source text");
    check(*first.metadata == *second.metadata &&
    ...
    const auto blank_source = ensure_retail_localization_metadata(
        root, "install-test-v1", "loc-parser-v1", "ff.loc.blank.v1", [] {
          return std::optional<std::vector<RetailSourceString>>{
              std::vector<RetailSourceString>{
                  {0U, ""}, {1U, "Project-authored nonblank source"}}};
        });
    check(blank_source.status == RetailLocalizationCacheStatus::extracted &&
              blank_source.metadata &&
              blank_source.metadata->ordinal_count == 2U,
          "private source cache preserves blank records without exposing them");
    check(make_retail_string_id("ff.loc.startup.v1", 9U) ==
              "off.retail.ff.loc.startup.v1.9",
    ...
    const auto translations = RetailTranslationCatalog::build(
        {{"off.retail.ff.loc.startup.v1.0",
          "Independently authored translation"},
         {"off.retail.ff.loc.startup.v1.1",
          "Another independently authored translation"}});
    check(translations &&
              translations->find("off.retail.ff.loc.startup.v1.1") &&
              !translations->find("off.retail.ff.loc.startup.v1.2"),
          "translation catalog resolves IDs without retaining source text");
    const auto bound_translations =
    ...
    check(bound_translations &&
          ...
          "ID-only translations bind to the extracted source set and ordinal "
          "range");
    check(!RetailTranslationCatalog::build_for_source_set(
              "ff.loc.startup.v1", 2U,
              {{"off.retail.ff.loc.startup.v1.2", "Out-of-range translation"}}),
          "ID-only translations reject an ordinal absent from the private "
          "extraction");
    check(!RetailTranslationCatalog::build_for_source_set(
              "ff.loc.startup.v1", 2U,
    ...
    check(!invalid_utf8, "translation catalog rejects invalid UTF-8 rather "
                         "than guessing an encoding");
    unsigned alternate_extractions{};
    const auto alternate = ensure_retail_localization_metadata(
        root, "install-test-v1", "loc-parser-v1", "ff.loc.menu.v1", [&] {
          ++alternate_extractions;
          return std::optional<std::vector<RetailSourceString>>{
              std::vector<RetailSourceString>{
                  {0U, "Project-authored alternate source"}}};
        });
    check(alternate.status == RetailLocalizationCacheStatus::extracted &&
              alternate_extractions == 1U,
          "different source-set identity never reuses an incompatible source "
          "catalog");
    const auto invalid = ensure_retail_localization_metadata(
        root, "install-test-v2", "loc-parser-v1", "ff.loc.startup.v1", [] {
          return std::optional<std::vector<RetailSourceString>>{
              std::vector<RetailSourceString>{{1U, "wrong ordinal"}}};
        });
    ...
    const auto loc_first = ensure_retail_localization_metadata(
    ...
    check(loc_first.status == RetailLocalizationCacheStatus::extracted &&
              loc_first.metadata && loc_first.metadata->ordinal_count == 2U &&
              loc_extraction_calls == 1U,
          "synthetic LOC records feed the local-only cache in decoder order");
    const auto loc_second = ensure_retail_localization_metadata(
        root, "install-test-v1", off::data::loc_catalog_parser_identity,
        "loc-source-synthetic-v1", [&] {
          ++loc_extraction_calls;
          return synthetic_loc_records();
        });
    check(loc_second.status == RetailLocalizationCacheStatus::loaded &&
              loc_second.metadata &&
              *loc_first.metadata == *loc_second.metadata &&
              loc_extraction_calls == 1U,
          "matching LOC source-set identity reuses private catalog metadata "
