...
Fixture catalog_fixture(bool include_flagged_reference = false) {
    ...
    append_u32(fixture.bytes, static_cast<std::uint32_t>(fixture.descriptor_offset));
    if (include_flagged_reference) {
        append_u32(
            fixture.bytes,
            0x80000000U | static_cast<std::uint32_t>(fixture.reference_offset)
        );
    }
...
int main() {
    ...
    check(catalog.base_data_end() == fixture.index_offset, "preserve base-data boundary");
    check(catalog.entries().size() == 1, "parse primitive index");
    const auto& entry = catalog.entries().front();
    check(entry.descriptor_offset == fixture.descriptor_offset, "preserve descriptor offset");
    ...
    const auto reference_catalog = off::data::PrimitiveCatalog::parse(
        reference_fixture.bytes
    );
    ...
    set_u32(
        overlapping_fixture.bytes,
        overlapping_fixture.index_offset + 4,
        0x80000000U |
            static_cast<std::uint32_t>(overlapping_fixture.descriptor_offset + 2)
    );
    bool overlapping_rejected = false;
    try {
        static_cast<void>(off::data::PrimitiveCatalog::parse(overlapping_fixture.bytes));
    } catch (const std::runtime_error&) {
    ...
    check_rejected(
        [](auto& value) { set_u32(value.bytes, 12, 2); },
        "reject an index table outside the file"
    );
    check_rejected(
        [](auto& value) { set_u32(value.bytes, value.index_offset, 0x40000000U); },
    ...
    check_rejected(
        [](auto& value) { set_u16(value.bytes, value.descriptor_offset + 14, 0); },
