...
int main() {
  ...
  static_assert(
      !std::is_constructible_v<NormalIntroScenePostReaderHostLifecycleServices,
  ...
  const auto host = NormalIntroSceneHostFactory::create(evidence(), services(),
                                                        {false, false, 44});
  ...
  check(rejects([&] {
          static_cast<void>(NormalIntroSceneHostFactory::create(
  ...
  activation_services.movie_control_phase_two = {[] { return false; }, [] {},
                                                 [](bool) {},          [] {},
                                                 [] { return 1U; },    [] {}};
  {
    int tail{}, lifecycle{};
    ...
    check(rejects([&] { failed_reader.activate(activation_services); }) &&
              ...
              tail == 0 && lifecycle == 0,
          "failed reader service leaves the host inactive before tail or "
          "lifecycle work");
  }
  {
    ...
    check(rejects([&] { failed_tail.activate(activation_services); }) &&
          ...
          "failed tail service leaves the host inactive before global "
          "lifecycle work");
  }
  {
    int reader{}, tail{};
    ...
    check(rejects([&] { failed_lifecycle.activate(activation_services); }) &&
              ...
              reader == 1 && tail == 1,
          "failed global lifecycle service leaves the host inactive before "
          "MovieControl phase two");
  }
